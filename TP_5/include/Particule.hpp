...
class Particule {
    ...
    public:
        ...
        Particule(std::vector<double> pos, std::vector<double> vitess, std::vector<double> force, double m, int Id, Categorie cat) 
            : position(pos), vitesse(vitess), force(force), m(m), Id(Id), Cat(cat) {}

        Particule() = default;

        int getDim() const { return position.size(); } // la fct ne modifie pas l’objet sur lequel elle est appelée.
