...
class Univers { 
    ...
    public : 
           Univers(int dim, int nb_part, double Lx, double Ly, double rcut, std::vector<Cellule> nos_Cellules)
            ...
            void initialiserVoisins(int ncx, int ncy) {
                    for (int y = 0; y < ncy; ++y) {
                        for (int x = 0; x < ncx; ++x) {
                            int currentIdx = x + y * ncx;
                            ...
                            for (int dy = -1; dy <= 1; ++dy) {
                                for (int dx = -1; dx <= 1; ++dx) {
                                    ...
                                    if (nx_voisin >= 0 && nx_voisin < ncx && ny_voisin >= 0 && ny_voisin < ncy) {
                                        int voisinIdx = nx_voisin + ny_voisin * ncx;
